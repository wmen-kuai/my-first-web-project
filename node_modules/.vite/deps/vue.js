      ...
      if (props) {
        if (true) {
          ...
          for (const key in props) {
            ...
            key[0] === "." || isCustomElement && !isReservedProp2(key)) {
...
function hasPropValueChanged(nextProps, prevProps, key) {
  ...
  if (key === "style" && isObject2(nextProp) && isObject2(prevProp)) {
...
var isDate2 = (val) => toTypeString3(val) === "[object Date]";
    ...
    for (const key in next) {
      ...
      if (value != null) {
        if (!shouldPreserveTextareaResizeStyle(
          el,
          key,
          !isString3(prev) && prev ? prev[key] : void 0,
          value
        )) {
...
function shouldPreserveTextareaResizeStyle(el, key, prev, next) {
  return el.tagName === "TEXTAREA" && (key === "width" || key === "height") && isString3(next) && prev === next;
}
var xlinkNS = "http://www.w3.org/1999/xlink";
function patchAttr(el, key, value, isSVG, instance, isBoolean = isSpecialBooleanAttr3(key)) {
  if (isSVG && key.startsWith("xlink:")) {
    ...
    if (value == null || isBoolean && !includeBooleanAttr2(value)) {
      ...
      el.setAttribute(
        key,
        isBoolean ? "" : isSymbol3(value) ? String(value) : value
      );
    }
  }
}
...
var patchProp = (el, key, prevValue, nextValue, namespace, parentComponent) => {
  ...
  if (key === "class") {
    ...
    el._def.__asyncLoader && (/[A-Z]/.test(key) || !isString3(nextValue)))
  ) {
...
function shouldSetAsPropForVueCE(el, key) {
  ...
  return Array.isArray(props) ? props.some((prop) => camelize3(prop) === camelKey) : Object.keys(props).some((prop) => camelize3(prop) === camelKey);
}
var REMOVAL = {};
function defineCustomElement(options, extraOptions, _createApp) {
  let Comp = defineComponent(options, extraOptions);
  if (isPlainObject3(Comp))
    Comp = extend3({}, Comp, extraOptions);
